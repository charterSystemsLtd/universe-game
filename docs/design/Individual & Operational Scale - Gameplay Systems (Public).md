...
- **Three depth layers — Near Space, Far Space, Deep Space** — each independently configured on three axes:
  ...
  - **Object size** (stars scattered within each layer): **Near largest, Far medium, Deep smallest** — reinforces the same near/far depth read visually, independent of movement.
  - Deep Space additionally includes an occasional very-low-opacity large placeholder shape standing in for a nebula/supernova — background visual interest, not yet real art. This one's genuinely fine as pure decoration; nothing to visit there.
  - **Deliberately zero decorative "planets" in any layer (corrected 2026-08-25).** An earlier first pass included planet-sized dots scattered through Near/Far space; this was wrong on two separate counts, not just one: (1) any major body the player can actually see should be a real, visitable entity handled by the Planetary Generation System, not a parallax object with no substance behind it; (2) independent of that, our own numbers (500×500 sm coordinates, only 75% spawn chance, region-scale distances in the thousands of sm) mean planets should be genuinely rare at any given moment — seeing many after barely moving directly contradicts the vastness the design calls for. Real visitable planets get wired in later per those actual numbers (development roadmap step 7, the sm-consistency audit), not as a quick re-enable of decorative dots.
...
- **Status:** movement/tiling mechanics implemented and functional. Visual content (star shapes/colors, nebula placeholder) explicitly first-pass/unverified — revisit once real art exists. Planet density corrected to zero per the above; not just deferred, actively wrong to re-add casually.
