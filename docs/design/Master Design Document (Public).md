...
## The three scales
A player moves between three distinct zoom levels, each a genuinely different mode of play, not just a visual zoom. See [Scale Terminology (Public)](Scale%20Terminology%20(Public).md) for the full reference table (fixed internal terms vs. situational UI labels). Summary:
...
**Yin and Yang have real nodes on the chart:** both sit on the pure horizontal line through the hexagon's true center, symmetric left/right, in close toward the middle. Styled distinctly (gold-bordered) from the six society nodes to read as conceptually different — forces, not societies. Visual: see the Societal Guide's Relationship Wheel section for the actual chart.
