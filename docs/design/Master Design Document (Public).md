...
## The three scales
A player moves between three distinct zoom levels, each a genuinely different mode of play, not just a visual zoom. See [[03 Active Projects/Universe Game/Design/Public/Scale Terminology (Public).md]] for the full reference table (fixed internal terms vs. situational UI labels). Summary:
...
**Yin and Yang have real nodes on the chart:** both sit on the pure horizontal line through the hexagon's true center, symmetric left/right, in close toward the middle. Styled distinctly (gold-bordered) from the six society nodes to read as conceptually different — forces, not societies. Visual: [[03 Active Projects/Universe Game/Design/assets/relationship-wheel.png]] (internal asset — not part of this sanitized doc set).
...
- **"Universe Game"** — the project's working title (renamed 2026-08-25), deliberately plain/generic. Not flagged as urgently needing replacement the way the prior title was — open to revisiting if something more distinctive comes along later, but this is the real working name for now.
